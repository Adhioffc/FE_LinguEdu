...
@section('content')
    ...
    @php
        // fallback kalau route belum kirim variabel
        $user = $user ?? Auth::user();
        $courseName = $courseName ?? null;
        $packageName = $packageName ?? null;
        $languageName = $languageName ?? null;
        $progress = $progress ?? 0;
        $levelLabel = $levelLabel ?? 'Level 1: Beginner';
    @endphp
    ...
    <style>
        ...
        .modern-card {
            ...
            border-radius: 24px;
            padding: 2rem;
            border: 1px solid rgba(11, 10, 10, 0.8);
        ...
        .modern-card:hover {
            ...
            background: rgba(57, 109, 213, 0.95);
        ...
        .theme-blue .btn-card-action {
            background: #eff6ff;
            color: #3b82f6;
        }

        .theme-blue:hover .btn-card-action {
            background: #3b82f6;
        ...
        .theme-green .btn-card-action {
            background: #f0fdf4;
            color: #22c55e;
        }

        .theme-green:hover .btn-card-action {
            background: #22c55e;
            color: white;
        }
        ...
        .theme-orange .btn-card-action {
            background: #fff7ed;
            color: #f97316;
        }

        .theme-orange:hover .btn-card-action {
            background: #f97316;
            color: white;
        }
    ...
    <aside class="modern-sidebar">
        <div class="d-flex align-items-center gap-3 mb-5 px-2">
            <div
                style="width: 40px; height: 40px; background: linear-gradient(135deg, #6366f1, #a855f7); border-radius: 10px; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.2rem; box-shadow: 0 4px 10px rgba(99, 102, 241, 0.4);">
    ...
    <div class="main-content">
        <div class="dashboard-grid">
            ...
            {{-- CARD WELCOME – tampilan sama, isi dinamis --}}
                ...
                <div style="position: relative; z-index: 1;">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            {{-- Info kursus aktif (kalau ada) --}}
                            @if ($courseName)
                                <p class="small mb-1 opacity-75">Kursus Aktif</p>
                                <div class="mb-2">
                                    <div class="fw-semibold">
                                        {{ $courseName }}
                                    </div>
                                    <small class="opacity-80">
                                        Bahasa: {{ $languageName }} • Paket: {{ $packageName }}
                                    </small>
                                </div>
                            @endif
                        </div>
                        <div
                            style="background: rgba(255,255,255,0.2); padding: 5px 12px; border-radius: 20px; font-size: 0.85rem;">
                    ...
                    <div style="margin-top: 2rem;">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <span class="fw-bold fs-5">{{ $levelLabel }}</span>
                        </div>
                    </div>
                </div>
            </div>
