...
@section('content')
    ...
    <div class="relative bg-blue-600 text-white text-center py-24 overflow-hidden">
        <div class="max-w-3xl mx-auto px-6 relative z-10">
            <h1 class="text-5xl font-extrabold mb-4 animate-fadeIn">Belajar Bahasa Dunia dengan <span
                    class="text-yellow-300">LinguEdu</span></h1>
        ...
        <div
            class="absolute inset-0 bg-gradient-to-b from-blue-600 via-blue-700 to-blue-800 opacity-40 pointer-events-none z-0">
        </div>
    </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-11/12 md:w-2/3 lg:w-1/2 p-8 relative animate-fadeSlide">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <a href="{{ route('login') }}"
                    ...
                    <span class="mt-4 inline-block text-sm text-blue-500 font-semibold group-hover:underline">Login Sekarang
                        →</span>
                </a>
                ...
                <a href="{{ route('register.simulasi') }}"
                    ...
                    <span class="mt-4 inline-block text-sm text-yellow-500 font-semibold group-hover:underline">Daftar
                        Sekarang →</span>
                </a>
            </div>
        </div>
    ...
    <div class="bg-white py-16">
        <div class="container mx-auto px-4">
            ...
            <div class="grid md:grid-cols-4 gap-8">
                <div
                    class="text-center p-6 hover:bg-blue-50 rounded-xl transition-all transform hover:-translate-y-2 group">
                    ...
                    <p class="text-gray-600 text-sm opacity-0 group-hover:opacity-100 transition">Dapatkan sertifikat resmi
                        diakui dunia.</p>
                </div>
                <div
                    class="text-center p-6 hover:bg-blue-50 rounded-xl transition-all transform hover:-translate-y-2 group">
                    ...
                    <p class="text-gray-600 text-sm opacity-0 group-hover:opacity-100 transition">Belajar langsung dari
                        pengajar luar negeri.</p>
                </div>
                <div
                    class="text-center p-6 hover:bg-blue-50 rounded-xl transition-all transform hover:-translate-y-2 group">
                    <div class="text-4xl mb-4 group-hover:scale-110 transition">📱</div>
                    <h3 class="font-bold mb-2 group-hover:text-blue-600">Fleksibel</h3>
                    <p class="text-gray-600 text-sm opacity-0 group-hover:opacity-100 transition">Akses kelas kapan pun, di
                        mana pun.</p>
                </div>
                <div
                    class="text-center p-6 hover:bg-blue-50 rounded-xl transition-all transform hover:-translate-y-2 group">
                    ...
                    <p class="text-gray-600 text-sm opacity-0 group-hover:opacity-100 transition">Koleksi materi terkurasi
                        oleh ahli bahasa.</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-blue-50 py-16">
        <div class="container mx-auto px-4">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                @foreach ([['flag' => 'gb', 'nama' => 'Bahasa Inggris', 'detail' => ['TOEFL', 'IELTS', 'Business English', 'Conversation']], ['flag' => 'jp', 'nama' => 'Bahasa Jepang', 'detail' => ['JLPT N5-N1', 'Kanji Master', 'Daily Conversation']], ['flag' => 'kr', 'nama' => 'Bahasa Korea', 'detail' => ['TOPIK I & II', 'K-Drama Korean', 'Hangul Master']]] as $lang)
                    <div
                        class="bg-white rounded-xl p-6 shadow-lg hover:shadow-2xl transition-all transform hover:-translate-y-2 relative group">
                        <img src="https://flagcdn.com/w160/{{ $lang['flag'] }}.png" alt="{{ $lang['nama'] }}"
                            class="w-32 mb-4 mx-auto hover:scale-110 transition">
    ...
    <div class="grid md:grid-cols-3 gap-8">
        @forelse ($paket as $index => $p)
            @php
                $nama = $p['nama_paket'] ?? 'Paket ' . ($index + 1);
                $isFav = strtolower($nama) === 'intermediate'; // tandai paket favorit

                // bullet default berdasarkan nama paket (kalau di DB belum ada deskripsi)
                $defaultBullets = match (strtolower($nama)) {
                    'basic' => [
                        '🔹 8x Modul Video Interaktif',
                        '🔹 Latihan Soal Setiap Bab',
                        '🔹 Akses Quiz Otomatis dan Skor Langsung',
                        '🔹 Sertifikat Level Dasar (CEFR A1–A2)',
                        '🔹 Durasi akses 3 bulan',
                        '🔹 Cocok untuk pemula tanpa pengalaman',
                    ],
                    'intermediate' => [
                        '⭐ 12x Modul Terstruktur + Challenge Mingguan',
                        '⭐ Simulasi Listening & Reading Adaptif',
                        '⭐ Sistem Penilaian Otomatis per Level',
                        '⭐ Sertifikat Level Menengah (CEFR B1–B2)',
                        '⭐ Durasi akses 6 bulan',
                        '⭐ Fitur “Retry Quiz” & Progress Tracking Realtime',
                        '⭐ Unlock Badge & Leveling Achievement',
                    ],
                    default => [
                        '🏅 Modul advanced + ujian akhir',
                        '🏅 Akses semua level sebelumnya',
                        '🏅 Sertifikat resmi LinguEdu',
                        '🏅 Simulasi tes global',
                        '🏅 Durasi akses panjang + laporan hasil ujian',
                    ],
                };

                // kalau di DB ada kolom "deskripsi" (teks multi-baris), kita pecah jadi bullet
                $desc = $p['deskripsi'] ?? '';
                $bullets = [];
            ...
            <div
                class="{{ $isFav
                    ? 'border-2 border-yellow-400 rounded-xl p-6 hover:border-yellow-500 bg-yellow-50 transition-all transform hover:-translate-y-2 hover:shadow-xl relative'
                    : 'border-2 border-blue-200 rounded-xl p-6 hover:border-blue-400 transition-all transform hover:-translate-y-2 hover:shadow-xl' }}">
                @if ($isFav)
                    <div
                ...
                <h3 class="text-2xl font-bold mb-4 {{ $isFav ? 'hover:text-yellow-600' : 'hover:text-blue-600' }}">
                    {{ $nama }}
                </h3>
                ...
                <a href="{{ route('register.simulasi') }}"
                    class="block text-center py-3 {{ $isFav ? 'bg-yellow-400 text-gray-900 hover:bg-yellow-500' : 'bg-blue-600 text-white hover:bg-blue-700' }} rounded-lg transition">
                    Pilih Paket {{ $nama }}
                </a>
            </div>
        @empty
            <p class="col-span-3 text-center text-gray-500">
                Belum ada paket yang terdaftar di sistem.
            </p>
        @endforelse
    </div>
